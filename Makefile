...
#CC = cc
CC = gcc -m32 -Wall -Wno-unused -MD
# because purify spits out warnings on stdout...
CC-DEP = gcc -m32 $(LIBPARAM_CFLAGS) $(DISKMODEL_CFLAGS)
...
all: disksim rms hplcomb syssim ycsim

clean:
	rm -f TAGS *.o disksim syssim rms hplcomb core libdisksim.a ycsim
...
#for clean my code
ycclean:
	rm -f ycsim.o yc_debug.o yc_syssim.o yc_ipc.o yc_caching.o 
...
DISKSIM_SRC = disksim.c disksim_intr.c disksim_pfsim.c \
	...
	raw_layout.c \
	yc_debug.c yc_syssim.c yc_ipc.c yc_caching.c 
...
ycsim: ycsim.o libdisksim.a yc_syssim.o
	$(CC) $(CFLAGS) -o $@ ycsim.o disksim_interface.o $(LDFLAGS) -pthread
