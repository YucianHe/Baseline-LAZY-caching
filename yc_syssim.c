...
void exec_SSDsim(char *name, const char *parm_file, const char *output_file) {
	...
	for (;;)
	{
		...
		if (rp->reqFlag == MSG_REQUEST_CONTROL_FLAG_FINISH)
		{
			...
			fprintf(result, "[SSDSIM]System Time:%lf, Pending IO Requests:%lu, Served IO Requests:%lu\n", st.stime, st.pendIORequest, st.servedIORequest);
...
void exec_HDDsim(char *name, const char *parm_file, const char *output_file) {
	...
	for (;;) 
	{
		...
		if (rp->reqFlag == MSG_REQUEST_CONTROL_FLAG_FINISH) {
			...
			fprintf(result, "[HDDSIM]System Time:%lf, Pending IO Requests:%lu, Served IO Requests:%lu\n", st.stime, st.pendIORequest, st.servedIORequest);
